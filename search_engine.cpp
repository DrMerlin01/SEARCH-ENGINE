...
class SearchServer {
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if ((document_id < 0) || (documents_.count(document_id) > 0)) {
            throw invalid_argument("Проверьте идентификатор документа, возможно он уже существует или имеет недопустимое значение.");
        }
    ...
    QueryWord ParseQueryWord(string text) const {
        if (text.empty()) {
            throw invalid_argument("Запрос не содержит слов.");
        }
